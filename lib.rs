...
fn import_work_dir(conn: &Connection, dir_path: &str) -> Result<i64, String> {
    ...
    let (title, studio, synopsis, tag_map, episode_list, characters) = if let Some(jp) = json_path {
        ...
        (m.title, m.studio.unwrap_or_default(), m.synopsis.unwrap_or_default(),
         m.tag.unwrap_or_default(), m.episode_list.unwrap_or_default(), m.characters)
    } else {
        (dir_name.clone(), String::new(), String::new(), std::collections::HashMap::new(), Vec::new(), None)
    };
...
fn write_work_json(work_id: i64, db: State<Database>) -> Result<String, String> {
    ...
    // Get episode count
    let ep_count: i32 = conn.query_row("SELECT COUNT(*) FROM Episodes WHERE WorkId=?1", params![work_id], |r| r.get(0)).unwrap_or(0);
...
#[tauri::command]
fn scan_folder(root_path: String, _db: State<Database>) -> Result<Vec<String>, String> {
    ...
    if let Ok(entries) = std::fs::read_dir(path) {
        for entry in entries.flatten() {
            ...
            if has_video || has_data {
                ...
                found.push(dir_path.to_string_lossy().to_string());
            }
        }
    }
    Ok(found)
...
fn batch_import_folders(folders: Vec<String>, db: State<Database>) -> Result<i32, String> {
    ...
    let mut count = 0;
    for fp in &folders {
        let name = std::path::Path::new(fp).file_name().and_then(|n| n.to_str()).unwrap_or("").to_string();
        if name.is_empty() { continue; }
        conn.execute("INSERT INTO Works (Title,Year,Month,Studio,FolderPath) VALUES (?1,2024,1,'',?2)", params![name,fp]).map_err(|e| e.to_string())?;
        let wid = conn.last_insert_rowid();
        if let Ok(entries) = std::fs::read_dir(fp) {
            let mut videos: Vec<_> = entries.flatten().filter(|e| e.path().is_file()).map(|e| e.path())
                .filter(|p| p.extension().and_then(|e| e.to_str()).map(|s| matches!(s.to_lowercase().as_str(), "mp4"|"mkv"|"avi"|"wmv"|"flv"|"mov"|"webm")).unwrap_or(false)).collect();
            videos.sort();
            for (i,v) in videos.iter().enumerate() {
                conn.execute("INSERT INTO Episodes (WorkId,Number,Title,VideoPath) VALUES (?1,?2,?3,?4)", params![wid,(i+1)as i32, format!("第{}集",i+1), v.to_string_lossy().to_string()]).map_err(|e| e.to_string())?;
            }
        }
        auto_generate_cover(&conn, wid, fp);
        count += 1;
    }
    Ok(count)
}
