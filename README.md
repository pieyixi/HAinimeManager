...
└─ src-tauri
   ...
   └─ src
      ...
      ├─ portable_library.rs  # 媒体库标记、自动定位、路径重绑定
      ├─ import.rs            # 按标准目录导入作品
...
媒体文件所在目录由 `portable_library.rs` 管理。首次使用现有数据库时，软件会在媒体库根目录创建：

```text
.hanime-library.json
```

文件只保存格式版本和随机媒体库 ID，不包含作品信息。`AppSettings` 表保存媒体库 ID、上次绝对路径，以及媒体库相对于 EXE 的位置。

启动时按以下顺序定位媒体库：

1. 根据媒体库相对于 EXE 的位置查找，适用于管理器和媒体一起移动。
2. 检查上次使用的绝对路径。
3. 保留原目录结构，尝试其他盘符下的同一路径。
4. 自动定位失败时进入设置页，由用户填写新目录并点击“设为媒体库”。

检测到盘符或根目录变化后，会在一个 SQLite 事务中同时更新：
...
任意一步失败都会回滚整个事务。新目录没有媒体库标记时，至少一半现有作品目录必须能够匹配，才允许恢复标记并重绑定；这样可以避免把数据库误绑到空目录或无关目录。作品的 `data/meta.json` 内容不会因为路径重绑定而改写。

当前采用兼容迁移方案：数据库内部继续保存运行时绝对路径，但媒体库身份和根目录是独立配置。这样无需同时重写播放器、封面缓存、建档和备份流程，又能解决移动硬盘盘符变化。后续如果改成数据库只保存逻辑相对路径，所有文件访问仍应统一经过 `portable_library.rs`，不能在各模块自行拼接盘符。
